...
static double getCategoryBits(const b2Filter &f) {
    return (double)f.categoryBits;
}
static void setCategoryBits(b2Filter &f, double val) {
    f.categoryBits = (uint64_t)val;
}

static double getMaskBits(const b2Filter &f) {
    return (double)f.maskBits;
}
static void setMaskBits(b2Filter &f, double val) {
...
EMSCRIPTEN_BINDINGS(box2d) {
    ...
    // Adjust fields below only if your b2Capsule has these members
    ...
    class_<b2Filter>("b2Filter")
        .constructor()
        .property("categoryBits", &getCategoryBits, &setCategoryBits)
        .property("maskBits", &getMaskBits, &setMaskBits)
